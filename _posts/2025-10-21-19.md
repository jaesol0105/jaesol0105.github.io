...
- 이때 최상단 위젯에 배경색을 적용해주면 실체를 갖게되어 Spacer 부분을 클릭해도 `onTap`이 적용된다

<br>

## Column 화면 깨짐
`Row`와 `Column`은 자식에게 명확한 크기 제한을 주지 않으면 무한한 공간으로 간주하기 때문에 `Expanded` 등으로 제약을 줘야 한다
...
한줄 요약하면 Expanded가 하는 일은<br>
"스크린(부모 위젯)의 남은 공간 중 이만큼을 너한테 줄게, 그 높이(또는 너비) 안에서만 크기 정해"
